...
def convert_png_to_tfrecord(case: int, output_path):
...
def convert_tfrecord_to_png(input_path, output_path):
...
def main(args):
    ...
    if args.convert_to == 'tfrecord':
        if args.case != -1:
            convert_png_to_tfrecord(args.case, args.output_path)
    elif args.convert_to == 'png' :
        if args.input_path:
            convert_tfrecord_to_png(args.input_path, args.output_path)
...
if __name__ == '__main__':
    ...
    ''')
    parser.add_argument('--convert-to', default='tfrecord', choices=['tfrecord', 'png'])
